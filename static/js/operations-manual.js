...
const chapters = [
  ...
  {id:'menu-week',category:'menu',label:'메뉴 6',title:'6. 1주 교재',summary:'작은 범위로 모델·자료·품질을 먼저 검증',body:`
    <ol><li>Provider, 과정 길이, 주차, 대상, 학생용·강사용·통합 유형을 선택합니다.</li><li>사용할 참고자료 체크 상태를 확인합니다.</li><li><b>1주 분량 생성</b> 후 설명·프롬프트·실습·파일 저장 진행률을 확인합니다.</li><li>품질 점수와 경고를 읽고 결과를 열어 검수합니다.</li></ol><p>전체 교재보다 먼저 이 메뉴로 1주를 생성하는 것이 권장됩니다. 로컬 모델이 중단되면 안전 기본 교재로 복구될 수 있으므로 경고가 있으면 반드시 내용을 다시 확인합니다.</p>`},
  {id:'menu-book',category:'menu',label:'메뉴 7',title:'7. 전체 교재',summary:'전체 또는 선택 구간을 학생용·강사용으로 생성',body:`
    <p>12주·15주 중 시작 주차와 끝 주차를 정해 생성합니다. 결과는 Markdown 원본과 함께 PPTX·PDF·HWPX로 내려받을 수 있습니다.</p><ul><li>각 차시는 핵심 프롬프트 3개와 핵심 3·선택 3·도전 1의 실습 7개를 기준으로 검사합니다.</li><li>품질 점수 85점 미만 또는 치명 오류가 있으면 출판 가능 상태가 되지 않습니다.</li><li>AI 초안은 강사 검토와 승인 전 최종본으로 사용하지 않습니다.</li></ul>`},
  ...
  {id:'data-paths',category:'data',label:'데이터·백업',title:'데이터와 결과 파일 위치',summary:'무엇이 어디에 저장되는지 확인',body:`
    <div class="manual-table-wrap"><table><thead><tr><th>경로</th><th>내용</th><th>운영 주의</th></tr></thead><tbody><tr><td><code>data/studio.db</code></td><td>계정·세션·과정·교재·자료·벡터</td><td>가장 중요한 기본 DB</td></tr><tr><td><code>exports/books</code></td><td>교재 Markdown·PPTX·PDF·HWPX</td><td>DB 기록과 함께 보존</td></tr><tr><td><code>exports/summaries</code></td><td>자료별 Markdown 요약</td><td>자료 삭제 시 연계 파일 확인</td></tr><tr><td><code>exports/source_packs</code></td><td>Gemini Notebook ZIP</td><td>필요 시 재생성 가능</td></tr><tr><td><code>logs</code></td><td>시작·서버 표준 출력·오류</td><td>비밀정보 공유 전 마스킹</td></tr><tr><td><code>uploads</code></td><td>처리 중 임시 업로드</td><td>원본은 추출 후 삭제</td></tr></tbody></table></div>`},
